...
$clerkID =mysqli_query($conn, "SELECT userID FROM users WHERE username = '$username';");
$rowClerk = mysqli_fetch_array($clerkID, MYSQLI_ASSOC);

$clerkInserted = $rowClerk['userID'];

$submissionID = $_SESSION['submissionForDeliveryCourier'];
$ri = mysqli_query($conn, "SELECT branchID FROM employee WHERE username = '$username';");
$rowID = mysqli_fetch_array($ri, MYSQLI_ASSOC);
if (isset($_POST['buy_btn'])) {
            // delivery assignment, submission status delivered
            $courierID = $_POST['rowid'];

            if(!$conn->query("INSERT INTO deliveryassignment(clerkID, courierID, submissionID, status)
            VALUES ('$clerkInserted', '$courierID', '$submissionID', 'finished');"))
            {
                echo $conn->error;
            }
            //$res12 = mysqli_query($conn, "INSERT INTO deliveryassignment(clerkID, courierID, submissionID, status)
           // VALUES ('$clerkInserted', '$courierID', '$submissionID', 'finished');");

            $query = mysqli_query($conn, "UPDATE submission set status = 'delivered' WHERE  submissionID = '$submissionID';");
